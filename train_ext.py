...
from utils import *
from loss import OriTripletLoss, TripletLoss_WRT, KLDivLoss, TripletLoss_ADP, SCLoss, CALoss
...
parser.add_argument('--workers', default=4, type=int, metavar='N',
                    help='number of data loading workers (default: mycode2)')
...
parser.add_argument('--margin', default=0.3, type=float,
                    metavar='margin', help='triplet myloss0.5 margin')
...
if dataset == 'sysu':
    data_path = '/home/xiegengsheng/wangguangzhou/datasets/SYSU-MM01/'
    log_path = args.log_path + 'sysu_log/'
    test_mode = [1, 2]  # thermal to visible
elif dataset == 'regdb':
    data_path = '/home/xiegengsheng/wangguangzhou/datasets/RegDB/'
    log_path = args.log_path + 'regdb_log/'
    test_mode = [2, 1]  # visible to thermal
elif dataset == 'llcm':
    data_path = '/home/xiegengsheng/wangguangzhou/datasets/LLCM/'
...
if len(args.resume) > 0:
    model_path = checkpoint_path + 'sysu_adp_joint_co_nog_ch_nog_sq1_aug_G_erase_0.5_p4_n8_lr_0.1_seed_0_best.t'
...
# criterion_DCL = SCLoss(num=2, weight=0.6)
criterion_DCL = SCLoss(num=3, weights=(0.6, 0.4), variance_aware=True)
criterion_CAL = CALoss(margin=0, num=2)
...
def train(epoch):
    ...
    for batch_idx, (input10, input11, input2, label1, label2) in enumerate(trainloader):
        ...
        # ========== 双路径SCL损失计算 ==========
        # 分割特征：1/3可见光，1/3过渡模态，1/3红外
        n = feat.size(0) // 3
        feat_v = feat[:n]  # 可见光分支1
        feat_t = feat[n:2 * n]  # 过渡模态（可见光分支2）
        feat_i = feat[2 * n:]  # 红外分支

        # 计算双路径损失 (V-T 和 I-T)
        loss_dcl = criterion_DCL(feat_v, feat_i, feat_t, label1)
        ...
        # 三元组损失（跨模态）
        loss_tri = (criterion_tri(torch.cat((feat_v, feat_t), 0), lbs)[0] +
                    criterion_tri(torch.cat((feat_v, feat_i), 0), lbs)[0] +
                    criterion_tri(torch.cat((feat_t, feat_i), 0), lbs)[0]) / 3
        batch_acc = criterion_tri(torch.cat((feat_v, feat_i), 0), lbs)[1]
        correct += (batch_acc / 2)

        # 跨模态对齐损失
        loss_cal = criterion_CAL(feat_v, feat_i, label1, label1)
        _, predicted = out0.max(1)
        correct += (predicted.eq(labels).sum().item() / 2)
        
       # pdb.set_trace()
        if epoch <= 100:
            loss = loss_id + loss_tri + args.kl * loss_kl + loss_dcl * 0.4
        else:
            loss = loss_id + loss_tri + args.kl * loss_kl + loss_cal
        ...
        dcl_loss.update(loss_dcl.item(), 2 * input1.size(0))
        cal_loss.update(loss_cal, 2 * input1.size(0))
        total += labels.size(0)
        ...
        if batch_idx % 50 == 0:
            print('Epoch: [{}][{}/{}] '
                  ...
                  'DCLoss: {dcl_loss.val:.4f} ({dcl_loss.avg:.4f}) '
                  'CALoss: {cal_loss.val:.4f} ({cal_loss.avg:.4f}) '
                  'Accu: {:.2f}'.format(
                epoch, batch_idx, len(trainloader), current_lr,
                100. * correct / total, batch_time=batch_time,
                train_loss=train_loss, id_loss=id_loss, tri_loss=tri_loss,kl_loss=kl_loss,dcl_loss=dcl_loss, cal_loss=cal_loss))

    writer.add_scalar('total_loss', train_loss.avg, epoch)
    writer.add_scalar('id_loss', id_loss.avg, epoch)
    writer.add_scalar('tri_loss', tri_loss.avg, epoch)
    writer.add_scalar('dcl_loss', dcl_loss.avg, epoch)
    writer.add_scalar('hc_loss', cal_loss.avg, epoch)
...
for epoch in range(start_epoch, 110):
    ...
    # training
    train(epoch)
    if epoch > 0 and epoch % 2 == 0 and epoch <= 100:
