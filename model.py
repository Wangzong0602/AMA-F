...
class embed_net(nn.Module):
    def __init__(self,  class_num, no_local= 'on', gm_pool = 'on', arch='resnet50' , num_stripes=6, probability=0.5):
        super(embed_net, self).__init__()

        self.thermal_module = thermal_module(arch=arch)
        self.visible_module = visible_module(arch=arch)
        self.mid_module = mid_module(arch=arch)
    ...
    def forward(self, x1, x2, modal=0):
        if modal == 0:
            x10, x11 = x1.chunk(2,0)
            x10 = self.visible_module(x10)
            x2 = self.thermal_module(x2)
            x11 = self.mid_module(x11)
            x = torch.cat((x10, x11, x2), 0)
            del x10, x11, x2
        elif modal == 1:
